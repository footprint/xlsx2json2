...
const plugins = parser.plugins; // footprint: 插件导出
...
function serializeWorkbook(parsedWorkbook, dest) {
  ...
  for (let name in parsedWorkbook) {
    ...
    // footprint: 替换特殊转义字符
    if (config.esc) {
      resultJson = resultJson.replace(/(\\)([n,r,t])/g, '$2');
    }
...
module.exports = {
  ...
  toJson: function (src, dest) {
    ...
    for (let i = 0; i < plugins.length; ++i) {
      if (typeof (plugins[i].serializeWorkbook) === 'function') {
        plugins[i].serializeWorkbook(parsed_workbook, dest);
      }
    }
