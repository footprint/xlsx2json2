...
if (typeof (config.plugins) !== 'undefined') {
  for (let pname in config.plugins) {
    var plugin = require("../plugins/" + pname);
...
function parseSheet(sheet, setting) {
  ...
  // footprint: 调用插件的 sheetBegin 方法
  for (let i = 0; i < plugins.length; ++i) {
    if (typeof (plugins[i].sheetBegin) === 'function') {
      plugins[i].sheetBegin(sheet, setting);
    }
  }
  ...
  for (let i_row = headIndex; i_row < sheet.data.length; i_row++) {
    ...
    // footprint: 调用插件的 parseRow 方法
    for (let i = 0; i < plugins.length; ++i) {
      if (typeof (plugins[i].parseRow) === 'function') {
        plugins[i].parseRow(row, i_row, setting.head);
      }
    }
    ...
    if (setting.type === SheetType.MASTER) {
      ...
      // footprint: 取出key
      let key = parsed_row[id_cell.name];
      // footprint: id列如果名为key则不写入对象中
      if (id_cell.name == "key") {
        delete parsed_row[id_cell.name];
      }
      // footprint：只有一列时展开对象，少一层嵌套
      let keys = Object.keys(parsed_row);
      if (keys.length == 1) {
        result[key] = parsed_row[keys[0]];
      } else {
  ...
  // footprint: 调用插件的 sheetEnd 方法
  for (let i = 0; i < plugins.length; ++i) {
    if (typeof (plugins[i].sheetEnd) === 'function') {
      plugins[i].sheetEnd(sheet, setting);
    }
  }
...
module.exports = {
  plugins: plugins, // footprint: 插件导出
  ...
  parseWorkbook: function (workbook, settings) {
    ...
    // footprint: 调用插件的 workbookBegin 方法
    for (let i = 0; i < plugins.length; ++i) {
      if (typeof (plugins[i].workbookBegin) === 'function') {
        plugins[i].workbookBegin(workbook, settings);
      }
    }
    ...
    // footprint: 调用插件的 workbookBegin 方法
    for (let i = 0; i < plugins.length; ++i) {
      if (typeof (plugins[i].workbookEnd) === 'function') {
        plugins[i].workbookEnd(workbook, settings);
      }
    }
