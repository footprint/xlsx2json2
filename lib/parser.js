...
function parseSheet(sheet, setting) {
  ...
  // footprint: 调用插件 sheetBegin 方法
  for (let i = 0; i < setting.head.length; ++i) {
    let name = setting.head[i].name;
    if (name.startsWith('&')) {
      var pname = name.substring(1, name.length);
      var plugin = require("../plugins/"+pname);
      plugin.sheetBegin(sheet, setting);
    }
  }
  ...
  // footprint: 调用插件 sheetEnd 方法
  for (let i = 0; i < setting.head.length; ++i) {
    let name = setting.head[i].name;
    if (name.startsWith('&')) {
      var pname = name.substring(1, name.length);
      var plugin = require("../plugins/"+pname);
      plugin.sheetEnd(sheet, setting);
    }
  }
...
function parseRow(row, rowIndex, head) {
  ...
  for (let index = 0; index < head.length; index++) {
    ...
    if (name.startsWith('&')) {
      var pname = name.substring(1, name.length);
      var plugin = require("../plugins/"+pname);
      plugin.parseRow(row, rowIndex, head);
      continue;
    }
...
module.exports = {
  ...
  parseWorkbook: function (workbook, settings) {
    ...
    // footprint: 加载所有插件并调用 workbookBegin 方法
    let plugins = config.plugins;
    if (typeof(plugins) !== 'undefined') {
      for (let pname in plugins) {
        var plugin = require("../plugins/"+pname);
        plugin.workbookBegin(workbook, settings);
      }
    }
    ...
    if (typeof(plugins) !== 'undefined') {
      for (let pname in plugins) {
        var plugin = require("../plugins/"+pname);
        plugin.workbookEnd(workbook, settings);
      }
    }
